...
func handle(next http.HandlerFunc, verbose bool) http.Handler {
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		log.Printf("%s - - [%s] \"%s %s %s\" \"%s\" \"%s\" \"%s\"", r.RemoteAddr, time.Now().Format("02/Jan/2006:15:04:05 -0700"), r.Method, r.URL.Path, r.Proto, r.Referer(), r.UserAgent(), r.Header.Get("X-Forwarded-For"))
	})
}
